...
///         Vault creator registers a fee, users pay to gain access, fees accumulate
///         in the contract and are withdrawable by the creator.
contract FixedFeeCondition {
    ...
    /// @notice Register a vault UUID with a fixed access fee.
    /// @param uuid The CDR vault UUID (from CDR.allocate)
    /// @param fee The fee in wei required to access the vault
